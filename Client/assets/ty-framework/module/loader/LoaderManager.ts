...
class Loader {
    ...
    public releaseCache(asset: Asset) {
        for (const [key, storedAsset] of this.assetMap) {
            if (storedAsset === asset) {
                //console.log("清理asset缓存", key, asset.name, asset.refCount);
    ...
    public releaseCacheByKey(_key: string) {
        for (const [key, storedAsset] of this.assetMap) {
            if (key === _key) {
                console.log("releaseCacheByKey", key, key);
                this.assetMap.delete(key);
                break;
            }
        }
    ...
    public preload(params: {
        ...
        return tyou.res.loader.preload(params);
    }
     ...
     * @param params.version 远程bundle的版本，参考https://docs.cocos.com/creator/manual/zh/asset/bundle.html#asset-bundle-%E7%9A%84%E7%89%88%E6%9C%AC
     */
    public preloadDir(params: {
        path: string,
        bundle?: string,
        version?: string,
        type?: typeof Asset,
        onProgress?: (finish: number, total: number, item: AssetManager.RequestItem) => void,
        onComplete?: (items: AssetManager.RequestItem[] | null) => void
    }) {
        return tyou.res.loader.preloadDir(params);
    }
     ...
     */
    public load<T extends typeof Asset>(params: {
        path: string,
        bundle?: string,
        version?: string,
        type?: T,
        ...
        if (this.assetMap.has(key)) {
            ...
            setTimeout(() => {
                ...
                console.log("缓存中取bundle", key, asset.name)
     ...
     * @param params.type 资源类型
     */
    public loadAsync<T extends typeof Asset>(params: {
        path: string,
        bundle?: string,
        version?: string,
        type?: T,
     ...
     * @param params.type 资源类型
     */
    public loadDir<T extends typeof Asset>(params: {
        path: string,
        bundle?: string,
        version?: string,
        type?: T,
        onProgress?: (finish: number, total: number, item: AssetManager.RequestItem) => void,
        onComplete?: (items: InstanceType<T>[] | null) => void
    }) {
        const key = `${params.bundle || 'resources'}-${params.type.name}-${params.path}-${params.version || ''}:`;
     ...
     * @param params.type 资源类型
     */
    public loadDirAsync<T extends typeof Asset>(params: {
        path: string,
        bundle?: string,
        version?: string,
        type?: T,
     ...
     */
    public loadRemote({url, ext, onComplete}: {
        url: string,
        ext?: string,
        onComplete?: (result: Asset | null) => void
    }) {
    ...
    public releaseAll() {
        ...
        // 延迟一秒释放资源
        setTimeout(() => {
            assetList.forEach(asset => tyou.res.decRef(asset));
        }, 1000);
    }
}
...
export class LoaderManager {
    ...
    private handle(handle: string, {bundle, version, path, type, onProgress, onComplete}: {
        bundle?: string,
        version?: string,
        path: string,
        type?: typeof Asset,
        ...
        args.push((err: string, res: any) => {
            if (err) {
                console.error(`${handle} "${path}" fail`, err);
