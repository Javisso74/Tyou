...
```typescript
// 打开窗口（支持传参）
await tyou.ui.showUIAsync(TestUI, arg1, arg2);
...
// 从外部关闭指定窗口
tyou.ui.closeWindow(TestUI);

// 从外部隐藏指定窗口
tyou.ui.hideWindow(TestUI);

// 关闭所有窗口
tyou.ui.closeAll();

// 查询窗口
tyou.ui.hasWindow(TestUI);          // 是否存在
tyou.ui.getWindow(TestUI);          // 获取实例
